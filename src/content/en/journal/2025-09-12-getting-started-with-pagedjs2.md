...
While Fred stayed in California, Gijs and I were in the office of Open Source Publishing (OSP) in Brussels, where we did quite a lot of things, around paged.js future. 
...
So we decided to look at [web components](https://developer.mozilla.org/en-US/docs/Web/API/Web_components) to make elements of the library bit more reusable.
And the first element we made is a `paged-page` component that works as a custom html element you can use if you want to have a proper print preview. It takes a `width` and a `height` attribute, and, it comes with all the needed css to handle print and preview. Each component has its own size: if you’re using Chrome, you can now have multiple page size in the same PDF. 

It’s still undocumented because we’re exploring the API design the pages: should they be components or `slots` to fill? Should we have `bleed` and `cropmarks` as custom component or are they set through an attribute?  
...
As you might know paged.js relies on [css-tree](https://github.com/csstree/csstree) to convert the CSS described in the standard to CSS supported by browsers. Through the houdini API browsers now have more and more native API's for manipulating CSS, mosty notably the CSS Object Model, also known as CSS OM. CSS OM is like the DOM for CSS. It allows for quick CSS inspection and manipulation.
...
One of the limitation of CSS OM is that you can only access the rendered stylesheet, the one the browser will use to draw things on the page. Where css-tree parses the stylesheet and provides and API to inspect it, CSS OM seems to provide access to the styleMap it has constructed based on the CSS it has parsed. So for example, there is no way to access comments written in the orignal stylesheet. Worse, if a CSS declaration has an unknown or unsupported property or value the declaration does not exist in the rendered stylesheet. The browser only manages the properties, values and selectors it knows. In the case of CSSPrint, we work with features that arent’t supported by browsers yet, exactly because paged.js wants to polyfill them, therefore, they just get removed. `string-set`? gone. `target-counter`? nada `@footnote`? lol.
...
- convert unknown values and property into CSS custom properties (that we often too quickly call css variables);
- replace unknown selectors into usable selectors before they get read by the browser.
